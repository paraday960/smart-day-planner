...
class IntelligentAssistantService {
  ...
  Future<String> _askOnline(String userText, List<Task> tasks) async {
    ...
    return SelfHealing.instance.guard(
      ...
      feature: 'online_ai',
      fallback: (e) {
        // اگر هوش آنلاین خطا داد، فعلاً یک پیام موقت بده
        // (self-healing در فراخوانی بعدی تلاش می‌کند)
  ...
  String _buildPrompt(String userText, String context) {
    ...
    return 'تو دستیار برنامه‌ریزی روزانهٔ هوشمند ایرانی هستی. همهٔ پاسخ‌ها فارسی، '
        ...
        'تاریخچهٔ گفتگو:\n$historyText\n'
        'سؤال کاربر: $userText\n'
        'خروجی: حداکثر ۵ خط فارسی، دوستانه و مفید. '
        'اگر سؤال به داده‌های برنامه (کار/مالی/بدهی/هدف) مربوط است و داده نیست، کوتاه بگو چه چیزی ثبت نشده و راهنمایی کن. '
        'برای سؤالات عمومی (فیلم/سلامتی/تغذیه/آب‌وهوا) بدون گفتن «در برنامه ثبت نشده» مستقیم پاسخ بده.';
  }
