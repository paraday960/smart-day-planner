...
## ۶. تکمیل: اتصال Feature Flags به کد

فاز ۳۹ قبلاً flagها را تعریف کرده بود ولی هیچ‌کدام به کد وصل نبودند؛ یعنی `--dart-define=ENABLE_CALENDAR=false` هیچ اثری نداشت. این نقص تکمیل شد:

- `lib/screens/home_screen.dart` حالا همه عملیات پلتفرمی را با flag مربوطه گیت می‌کند (تقویم، PDF، اشتراک فایل، هشدارهای هوشمند، بکاپ، فرمان و پاسخ صوتی). وقتی flag خاموش است، به‌جای اجرای عملیات، پیام «این نسخه غیرفعال است» نشان داده می‌شود.
- اگر `ENABLE_VOICE_INPUT=false` باشد، سرویس تشخیص صدا اصلاً راه‌اندازی نمی‌شود (در بیلد امن هیچ وابستگی به speech plugin وجود ندارد).
- تست `test/feature_gating_widget_test.dart` هر دو حالت (روشن/خاموش) را با dart-define بررسی می‌کند.
- `flutter_ci.yml` حالا گام جداگانه‌ای برای اجرای تست گیتینگ با همه flagهای خاموش دارد.
- `flutter analyze` از ۸۷ مورد به صفر رسید و `flutter test` (۳۳ تست + ۵ skip) پاس است.
